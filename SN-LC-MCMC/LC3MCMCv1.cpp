...
void data()			 /*Reading input file*/
 ...
 if(nifit!=0 && Ep!=0 && tp!=0){ printf("Magnetar is not compatible with NiAg fitting. Setting magnetar to 0."); Ep=0; tp=0;
 }
...
if(elso==1) fki << "# \n";
if(elso==1) fki << "# ID 	 R0 [cm]     M [M_sol]    Ek [erg]      Eth [erg]          v [cm/s]       T0 [K]        M_Ni [M_sol]    Ag [d^2]       kappa [g/cm^2]    s    Tion [K]       STD [mag], chi^2; whole and without tail\n";
...
if(elso==1) fki2 << "# ID 	 R0 [cm]     M [M_sol]    Ek [erg]      Eth [erg]          v [cm/s]       T0 [K]        M_Ni [M_sol]    Ag [d^2]       kappa [g/cm^2]    s    Tion [K]       STD [mag], chi^2; whole and without tail\n";
}
...
if(niag!=1) if(elso==0){
	...
	tv=read2(sor,6);
	tT0=read2(sor,7);
	tMni=read2(sor,8)*Msol;
	tAg=read2(sor,9)*(day*day);
	tkappa=read2(sor,10);
	ts=read2(sor,11);
	tTion=read2(sor,12);
	tszorasf=read2(sor,13)/2.5;
	szorash=tszoras=read2(sor,14)/2.5;
	tkhinegyzetf=read2(sor,15);
	khinegyzeth=tkhinegyzet=read2(sor,16);
}
