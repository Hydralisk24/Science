...
fprintf(gmean,"Eth-R0  parameter space\n");
...
fprintf(gsh,"#rm LC3p3.run\n");
fprintf(gsh,"g++ LC3p3.cpp -o LC3p3.run\n");
for(i=0;i<m;i++){
 			if(i==0) fprintf(gsh,"echo %s > par.inp\n",savestr[0].c_str());
 			if(i!=0) fprintf(gsh,"echo %s > par.inp\n",savestr2[randsor(i,qqq)].c_str());
 			fprintf(gsh,"./LC3p3.run 1\n");
