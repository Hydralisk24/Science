...
//double parb=0;	  // parameter b
					...
					double vref=0;  // mean, in [cm] e.g: 5e8
					double vrefstd=0; // error, in [cm] e.g: 1e8
...
double read(){
 ...
 f=fopen("parametersMC.inp","wt");	
   ...
   fprintf(f,"%d\t[Fit the nickel mass? (binary)]\n",1);	  /*nifit*/
...
// Chech if there are fitted Ni(Ag) function, if not, then the program fits
// niag chacges here
if(nifit!=0) niagread(); else{ niag=0; printf("No Ni fit!\n"); }
...
//fki.open("rand.out");
if(fkiS==NULL) elso=1; else elso=0;
...
if(niag==1){ 
			...
			printf("\nSampling the nebular phase to determinate the Ni mass:\n300000 steps, please do not interrupt.\n");
...
if(elso==1) fki << "# \n";
if(elso==1) fki << "# ID 	 R0 [cm]     M [M_sol]    Ek [erg]      Eth [erg]          v [km/s]       T0 [K]        M_Ni [M_sol]    Ag [d^2]       kappa [g/cm^2]    s    Tion [K]       STD [mag], chi^2; whole and without tail\n";
...
if(elso==1) fki2 << "# ID 	 R0 [cm]     M [M_sol]    Ek [erg]      Eth [erg]          v [km/s]       T0 [K]        M_Ni [M_sol]    Ag [d^2]       kappa [g/cm^2]    s    Tion [K]       STD [mag], chi^2; whole and without tail\n";
...
for(ciklus=0;ciklus<=int(NMC);ciklus++){
  ...
  khinegyzet=szoras;
  khinegyzetf=szorasf;
  szoras=sqrt(szoras/Tn);      //scatter beteen tmin and tfark; before tail
  szorasf=sqrt(szorasf/Tnf);   //whole scatter
  ...
  //if(accept==1) fkiS=fki; else fkiS=fki2;
    ...
    if(niag!=1){
	 ...
	 << " \t " << scientific << R0 << "   " << fixed << M/Msol << scientific << "   " << Ek << "   " << E 
	 << "      " << v << "   " << T0 << "   " <<  Mni/Msol << "   " << Ag/(day*day)
     ...
     if(accept==0){
        ...
        if(szorasf*0==0 && szoras*0==0) outtemp << setprecision(3) << ciklus 
	 	<< " \t " << scientific << tR0 << "   " << fixed << tM/Msol << scientific << "   " << tEk << "   " << tE 
	 	<< "      " << tv << "   " << tT0 << "   " <<  tMni/Msol << "   " << tAg/(day*day)
